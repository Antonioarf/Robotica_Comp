...
while(True):
    ...
    circles = None
    circles=cv2.HoughCircles(bordas,cv2.HOUGH_GRADIENT,2,40,param1=50,param2=75,minRadius=5,maxRadius=60)
    ...
    if circles is not None:
        ...
        for i in range(0,len(listax)-1):
            ...
            x = listax[i+1]
            y = listay[i+1]
            h = (fabs((x-x0)**2)  + fabs((y-y0)**2))**(1/2)
            H = 5.9
            f = 533.898
            D = H*(f/h)
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(bordas_color,'A distancia e: {} '.format(D),(0,50), font, 1,(255,255,255),2,cv2.LINE_AA)
